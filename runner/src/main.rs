...
async fn main() {
    ...
    match (args.task.clone(), args.action.clone()) {
        (Some(task), None) => {
            ...
            if let Some(task_def) = workspace.config.as_ref().unwrap().get_task(&task) {
                let (logs, success, task_output) = execute_task(&task_def.flow, &workspace.config.as_ref().unwrap(), &input).await;
        ...
        (None, Some(action_name)) => {
            info!("Running action: {} with job_id: {}, worker_id: {}", action_name, args.job_id, args.worker_id);
                ...
                if let Some(action_def) = workspace.config.as_ref().unwrap().get_action(&action_name) {
...
async fn execute_task(flow: &HashMap<String, common::workspace::FlowStep>, config: &WorkspaceConfiguration, input: &Option<Value>) -> (Vec<LogEntry>, bool, Option<Value>) {
    ...
    while let Some(step_name) = pending.pop() {
        ...
        if let Some(step) = flow.get(&step_name) {
            ...
            let (mut step_logs, step_success, step_output) = execute_action(config.get_action(action_name).unwrap(), &step_input).await;
